...
export default function App() {
  ...
  const handleCreateMarketingComment = async (postId: string, content: string, authorId: string) => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setMarketingComments(prev => [...prev, newComment]);
      }
    } catch (error: any) {
      ...
      setMarketingComments(prev => [...prev, newComment]);
      alert(`서버 저장 실패: 오프라인 모드로 로컬에만 추가되었습니다. (${error.message || '네트워크 확인 필요'})`);
    }
  };
  ...
  const handleUpdateMemo = async (id: string, content: string, tags: string[], authorId: string, projectId?: string) => {
    try {
      const { error } = await supabase
        .from('builder_memos')
        .update({ content, tags, author_id: authorId, project_id: projectId })
        .eq('id', id);
      
      if (error) throw error;
      setMemos(prev => prev.map(m => m.id === id ? { ...m, content, tags, authorId, projectId } : m));
    } catch (error: any) {
  ...
  const handleAddFeedback = async (projectId: string) => {
    ...
    // Use current member or default
    const feedbackAuthorId = members.length > 0 ? members[0].id : 'u1';
