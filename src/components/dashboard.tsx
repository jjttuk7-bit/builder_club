...
interface BoardProps {
  ...
  members: Member[];
  onDelete: (id: string) => void;
  onCreate: (content: string, authorId: string) => Promise<void> | void;
  onDeleteComment: (id: string) => void;
  onCreateComment: (postId: string, text: string, authorId: string) => Promise<void> | void;
  isDarkMode: boolean;
}
...
interface BuilderLogProps {
  ...
  projects: Project[];
  onDelete: (id: string) => void;
  onCreate: (content: string, tags: string[], authorId: string, projectId?: string) => Promise<void> | void;
  onUpdate: (id: string, content: string, tags: string[], authorId: string, projectId?: string) => Promise<void> | void;
  isDarkMode: boolean;
}
...
export const BuilderLog = ({
  ...
  const [viewingMemo, setViewingMemo] = React.useState<BuilderMemo | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await onUpdate(editingId, content, tags, selectedAuthorId, selectedProjectId || undefined);
      } else {
        await onCreate(content, tags, selectedAuthorId, selectedProjectId || undefined);
      }
      resetForm();
    } finally {
  ...
  const startEdit = (memo: BuilderMemo) => {
    setEditingId(memo.id);
    setContent(memo.content);
    setTagInput(memo.tags.join(', '));
    setSelectedAuthorId(memo.authorId);
    setSelectedProjectId(memo.projectId || '');
  ...
  return (
    <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto">
      <div className="flex items-center gap-3 mb-8 px-4">
        ...
        <div>
          <h2 className={`text-2xl font-black ${isDarkMode ? 'text-white' : 'text-slate-800'} tracking-tighter uppercase`}>
            {editingId ? '로그 수정하기' : '빌더 로그 (Builder\'s Log)'}
          </h2>
          <p className="text-slate-400 font-bold text-xs">{editingId ? '기존 로그 내용을 수정하고 태그를 관리하세요.' : '수행 과정에서의 짧은 생각, 이슈, 핵심 메모를 기록하세요.'}</p>
        </div>
      </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center">
              <button 
                ...
                className={`w-full sm:w-auto ${isSubmitting ? 'bg-slate-400' : 'bg-indigo-600 hover:bg-indigo-700'} text-white px-10 py-3.5 rounded-xl font-black shadow-lg shadow-indigo-500/20 active:scale-95 transition-all text-sm disabled:cursor-not-allowed`}
              >
                {isSubmitting ? '저장 중...' : (editingId ? '수정 완료' : '로그 남기기')}
              </button>
              {editingId && (
                <button 
                  type="button"
                  onClick={resetForm}
                  className={`w-full sm:w-auto px-10 py-3.5 rounded-xl font-black transition-all text-sm ${isDarkMode ? 'bg-slate-800 text-slate-400 hover:text-white' : 'bg-slate-100 text-slate-500 hover:text-slate-800'}`}
                >
                  취소
                </button>
              )}
        ...
        {memos.map(memo => {
          ...
          return (
            <motion.div 
              ...
              className={`p-6 rounded-3xl border relative group transition-all cursor-pointer ${isDarkMode ? 'bg-slate-900 border-slate-800 hover:border-indigo-800/50' : 'bg-white border-slate-200 hover:border-indigo-200 hover:shadow-md'}`}
            >
      ...
      <AnimatePresence>
        {viewingMemo && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-10 backdrop-blur-xl bg-slate-900/60 transition-all duration-500 overflow-y-auto">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className={`w-full max-w-3xl rounded-[3rem] shadow-2xl p-8 sm:p-12 relative my-auto ${isDarkMode ? 'bg-slate-900 border border-slate-800' : 'bg-white border border-slate-100'}`}
              onClick={e => e.stopPropagation()}
            >
              <button 
              ...
              <div className="flex items-center gap-4 mb-10">
                <img 
                  src={members.find(m => m.id === viewingMemo.authorId)?.avatarUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${viewingMemo.authorId}`} 
                  className="w-14 h-14 rounded-[1.5rem] border-2 border-indigo-500/20" 
                  alt="author"
                />
                <div>
                  <h3 className={`text-xl font-black ${isDarkMode ? 'text-white' : 'text-slate-800'} tracking-tight`}>
                    {members.find(m => m.id === viewingMemo.authorId)?.name || '익명 빌더'}
                  </h3>
                  <div className="text-sm text-slate-400 font-black flex items-center gap-3">
                    <span>{new Date(viewingMemo.createdAt).toLocaleString()}</span>
                    {viewingMemo.projectId && (
                      <span className="flex items-center gap-1.5 text-blue-500 uppercase tracking-widest text-[10px]">
                        <Briefcase className="w-3 h-3" />
                        {projects.find(p => p.id === viewingMemo.projectId)?.title}
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className={`prose prose-lg max-w-none mb-10 ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>
                <p className="text-2xl font-bold leading-relaxed whitespace-pre-wrap italic">
                  <LinkifyText text={viewingMemo.content} />
                </p>
              </div>

              <div className="flex flex-wrap gap-3 pt-8 border-t border-slate-100 dark:border-slate-800">
                {viewingMemo.tags.map(tag => (
                  <span key={tag} className={`text-xs font-black px-4 py-2 rounded-xl ${isDarkMode ? 'bg-slate-800 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}>
                    #{tag}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-12 flex justify-end gap-4">
                <button 
                  onClick={() => {
                    startEdit(viewingMemo);
                    setViewingMemo(null);
                  }}
                  className="flex items-center gap-2 px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black hover:bg-indigo-700 transition-all active:scale-95 shadow-lg shadow-indigo-500/20"
                >
          ...
          >
            <div className="flex flex-col h-full gap-10">
              ...
              <textarea 
                ...
                className={`flex-1 w-full bg-transparent border-none focus:ring-0 resize-none text-xl font-bold leading-[2rem] p-0 italic min-h-[1200px] ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}
                autoFocus
              />
...
export const Board = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      await onCreate(content, selectedAuthorId);
      setContent('');
    } finally {
      setIsSubmitting(false);
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end">
            <button 
              type="submit"
              disabled={isSubmitting}
              className={`${isSubmitting ? 'bg-slate-400' : 'bg-blue-600 hover:bg-blue-700'} text-white px-10 py-4 rounded-2xl font-black shadow-lg shadow-blue-500/20 transition-all active:scale-95 text-base disabled:cursor-not-allowed`}
            >
...
const CommentInput = ({ onSend, members, isDarkMode }: { onSend: (text: string, authorId: string) => Promise<void> | void, members: Member[], isDarkMode: boolean }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      await onSend(text, selectedAuthorId);
      setText('');
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-2">
      <select 
        ...
        onChange={(e) => setSelectedAuthorId(e.target.value)}
        disabled={isSubmitting}
        className={`px-3 py-2 rounded-xl border text-xs font-bold outline-none transition-all disabled:opacity-50 ${isDarkMode ? 'bg-slate-900 border-slate-700 text-slate-300' : 'bg-white border-slate-200 text-slate-600'}`}
      >
      ...
      <input 
        ...
        placeholder={isSubmitting ? "저장 중..." : "댓글을 남겨보세요..."}
        disabled={isSubmitting}
        className={`flex-1 px-4 py-2 rounded-xl border text-sm font-bold outline-none transition-all disabled:opacity-50 ${isDarkMode ? 'bg-slate-900 border-slate-700 text-slate-100 placeholder:text-slate-600 focus:border-blue-500' : 'bg-white border-slate-200 text-slate-800 placeholder:text-slate-300 focus:border-blue-500'}`}
      />
      <button 
        type="submit"
        disabled={isSubmitting}
        className={`${isSubmitting ? 'bg-slate-400' : 'bg-blue-600 hover:bg-blue-700'} text-white px-4 py-2 rounded-xl font-black text-sm shadow-md shadow-blue-500/10 active:scale-95 transition-all disabled:cursor-not-allowed`}
      >
        {isSubmitting ? '...' : '댓글 달기'}
      </button>
    </form>
